...
![Komunikasi dengan vs tanpa API](/image/api-without-vs-with.svg)

- **Tanpa API**: computer tidak bisa membaca tampilan visual dengan mudah
- **Dengan API**: computer bisa proses data langsung, otomatis, dalam detik
...
![Komunikasi Client-Server via API](/image/api-client-server.svg)
...
Setiap HTTP request terdiri dari 4 komponen utama:

1. **URL** — alamat endpoint, misalnya `https://api.example.com/customers/123`
2. **Method** — jenis operasi: `GET`, `POST`, `PUT`, atau `DELETE`
3. **Headers** — metadata request, misalnya `Content-Type: application/json` dan `Authorization: Bearer <token>`
4. **Body** — data yang dikirim (kosong untuk GET, berisi JSON untuk POST/PUT)
...
- Authorization: Basic cmlla3...cmQ=
...
Alur OAuth 2.0 berlangsung dalam beberapa langkah:

1. User klik tombol "Connect with [Service]" di aplikasi
2. Aplikasi redirect user ke halaman login Authorization Server
3. User login dan setujui permission yang diminta
4. Authorization Server redirect balik ke aplikasi dengan Auth Code
5. Aplikasi tukar Auth Code dengan Access Token (server-to-server)
6. Aplikasi simpan Access Token dan gunakan untuk request API selanjutnya
**SERVER:**
